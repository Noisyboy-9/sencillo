...
func TestCloudFirstScheduler_Filter(t *testing.T) {
...
func TestCloudFirstScheduler_Run(t *testing.T) {
	cases := map[string]struct {
		targetPod    model.Pod
		clusterNodes []model.Node
		...
		"cloud node is prioritized over edge nodes": {
			targetPod: model.Pod{
				Cores:  resource.NewQuantity(1, resource.DecimalSI),
				Memory: resource.NewQuantity(20*1024, resource.BinarySI),
			},
