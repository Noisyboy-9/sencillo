...
func TestBiggestFittingEdgeNodeScheduler_Filter(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			scheduler := BiggestFittingEdgeNodeScheduler{}
...
func TestBiggestFittingEdgeNodeScheduler_Run(t *testing.T) {
	cases := map[string]struct {
		targetPod    model.Pod
		clusterNodes []model.Node
		...
		"biggest edge node is prioritized over edge nodes": {
			targetPod: model.Pod{
				Cores:  resource.NewQuantity(1, resource.DecimalSI),
				Memory: resource.NewQuantity(20*1024, resource.BinarySI),
			},
			clusterNodes: []model.Node{
				...
				},
				{
					...
					AllocatableCores:  resource.MustParse("10"),
					AllocatableMemory: resource.MustParse("100Gi"),
					RemainingCores:    resource.MustParse("1"),
					RemainingMemory:   resource.MustParse("1Gi"),
					IsOnEdge:          true,
					IsMaster:          false,
				},
				{
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			s := BiggestFittingEdgeNodeScheduler{}
